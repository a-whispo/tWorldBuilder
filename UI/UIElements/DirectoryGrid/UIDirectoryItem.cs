...
{
    internal abstract class UIDirectoryItem : UIElement
    {
        ...
        public UIDirectoryItem(string fullPath)
        {
            FullPath = fullPath;
            ...
            _texture.TextureHover = ModContent.Request<Texture2D>("TerrariaInGameWorldEditor/UI/UIImages/TextureHover");
            Append(_texture);
            ...
            // delete
            _deleteButton = new TIGWEButton(ModContent.Request<Texture2D>("TerrariaInGameWorldEditor/UI/UIImages/Delete"));
            ...
            // rename
            _renameButton = new TIGWEButton(ModContent.Request<Texture2D>("TerrariaInGameWorldEditor/UI/UIImages/Rename"));
            ...
            _confirmRenameButton = new TIGWEButton(ModContent.Request<Texture2D>("TerrariaInGameWorldEditor/UI/UIImages/RenameConfirm"));
            ...
            // select
            _selectButton = new TIGWEButton(ModContent.Request<Texture2D>("TerrariaInGameWorldEditor/UI/UIImages/Load"));
            ...
            // icon
            _icon = new UIImage(ModContent.Request<Texture2D>("TerrariaInGameWorldEditor/UI/UIImages/File")); // just a placeholder texture, overridden in child classes
            _icon.IgnoresMouseInteraction = true;
            _icon.AllowResizingDimensions = false;
            Append(_icon);
        }

        public void AssignParentFolder(UIDirectoryFolder folder)
        {
            _parentFolder = folder;
        }

        public void AssignParentGrid(UIDirectoryGrid grid)
        {
            _parentGrid = grid;
        }
        ...
        public void StartDelete()
        {
            if (!_isDeleting)
            {
                _deleteButton.SetImage(ModContent.Request<Texture2D>("TerrariaInGameWorldEditor/UI/UIImages/DeleteConfirm"));
                _deleteButton.HoverText = "Confirm delete";
                _isDeleting = true;
            }
            else
            {
                if (this is UIDirectoryFile) // if its a file
                {
                    File.Delete(FullPath);
                }
                else if (this is UIDirectoryFolder folder) // if it's a directory
                {
        ...
        private void ConfirmRename()
        {
            ...
            if (!string.IsNullOrWhiteSpace(_renameTextField.GetText()))
            {
                ...
                try
                {
                    ...
                    FullPath = newPath;
                    if (this is UIDirectoryFolder folder) // recalculate the paths of all the folders children so they are still shown in order
                    {
