...
namespace TerrariaInGameWorldEditor.Common.Utils
{
    internal static class DrawUtils
    {
        ...
        { 
            ...
            int maxX = minX + (int)(Main.screenWidth * Main.UIScale) / 16;
            int minY = (int)(Main.screenPosition.Y / 16) - coordToDrawAt.Y;
            int maxY = minY + (int)(Main.screenHeight * Main.UIScale) / 16;
            ...
            try
            {
                ...
                for (int x = minX; x < maxX; x++)
                {
                    for (int y = minY; y < maxY; y++)
                    {
                        ...
                        if (!tilesToDraw.TryGetTile(tileCoord, out TileCopy tile))
                        {
                            continue;
                        }
                        int drawX = x * 16 + coordToDrawAt.X;
                        ...
                        // wall
                        if ((tile.WallType != 0) && drawWalls) // dont draw if there isnt a wall
                        {
                            ...
                            if (tileWallTex != null)
                            {
                                SpriteBatch.Draw(tileWallTex, boundsWall, new Rectangle(tile.WallFrameX, tile.WallFrameY, 32, 32), Color.White * 0.6f);
                            }
                        }
                    }
                }
                ...
                for (int x = minX; x < maxX; x++)
                {
                    for (int y = minY; y < maxY; y++)
                    {
                        ...
                        if (drawTiles)
                        {
                            ...
                            if (tile.IsTreeTop)
                            {
                                ...
                                else if (tile.TileType == 323) // palm treetops
                                {
                                    ...
                                    Rectangle boundsTile = new Rectangle(drawX - num1 + tile.TileFrameY, drawY + 16 + num2 - height, width, height);
                                    Texture2D top = ps.TryGetTreeTopAndRequestIfNotReady(treeTextureIndex, tile.TreeBiome, tile.TileColor);
                                    SpriteBatch.Draw(top, boundsTile, new Rectangle?(new Rectangle(tile.TreeFrame * (tile.TreeFrameWidth + 2), tile.TreeFrame * (tile.TreeFrameHeight + 2) + y2, width, height)), Color.White * 0.6f);
                                }
                                else
                                { // rest of the treetops
                                    Rectangle boundsTile = new Rectangle(drawX - tile.TreeFrameWidth / 2 + 8, drawY - tile.TreeFrameHeight + 16, tile.TreeFrameWidth, tile.TreeFrameHeight);
                                    Texture2D top = ps.TryGetTreeTopAndRequestIfNotReady(tile.TreeStyle, 0, tile.TileColor);
                                    SpriteBatch.Draw(top, boundsTile, new Rectangle?(new Rectangle(tile.TreeFrame * (tile.TreeFrameWidth + 2), 0, tile.TreeFrameWidth, tile.TreeFrameHeight)), Color.White * 0.6f);
                                }
                            }

                            // tree branches
                            if (tile.IsTreeBranch) // texture dimensions are kind of hardcoded so this might not work with modded trees
                            {
                                Texture2D branch = ps.TryGetTreeBranchAndRequestIfNotReady(tile.TreeStyle, 0, tile.TileColor);

                                // (40 is the width and height of the branch sprite)
                                if (tile.IsFlipped)
                                {
                                    Rectangle boundsTile = new Rectangle(drawX - 24, drawY - (40 / 2) + (16 / 2), 40, 40);
                                    SpriteBatch.Draw(branch, boundsTile, new Rectangle?(new Rectangle(42, tile.TreeFrame * 42, 40, 40)), Color.White * 0.6f, default, default, SpriteEffects.FlipHorizontally, default);
                                }
                                else
                                {
                                    Rectangle boundsTile = new Rectangle(drawX, drawY - (40 / 2) + (16 / 2), 40, 40);
                                    SpriteBatch.Draw(branch, boundsTile, new Rectangle?(new Rectangle(42, tile.TreeFrame * 42, 40, 40)), Color.White * 0.6f);
                                }
                            }

                            // tile
                            if (!tile.IsTreeTop && !tile.IsTreeBranch && tile.HasTile) // dont draw if theres no tile
                            {
                                // calculate bounds
                                Rectangle boundsTile = new Rectangle(drawX, drawY + (tile.IsHalfBlock ? 8 : 0), 16, (tile.IsHalfBlock ? 8 : 16));
                                ...
                                // get texture from spritesheet with help from frameX and frameY and draw it
                                if (tile.IsTreeTrunk && tile.TileType != 72 && tileTex != null) // this draws general trees, underground mushroom trees should be drawn like normal tiles tho
                                {
                                    ...
                                    SpriteBatch.Draw(tileTex, boundsTile, new Rectangle((176 * tile.TreeVariant) + tile.TileFrameX, tile.TileFrameY, 20, 16), Color.White * 0.6f);
                                }
                                else if (tile.TileType == 323) // this draws palm trees
                                {
                                    ...
                                    // get texture
                                    tileTex = ps.TryGetTileAndRequestIfNotReady(tile.TileType, tile.TreeBiome, tile.TileColor);

                                    // this determines the palm tree curve
                                    if (!(tile.TileFrameX <= 132 && tile.TileFrameX >= 88))
                                    {
                                        boundsTile.X += tile.TileFrameY;
                                    }
                                    SpriteBatch.Draw(tileTex, boundsTile, new Rectangle(tile.TileFrameX, 22 * tile.TreeBiome, 20, 16), Color.White * 0.6f);
                                }
                                else // draw normal tiles
                                {
                                    if (tileTex != null)
                                    {
                                        ...
                                        SpriteBatch.Draw(tileTex, boundsTile, new Rectangle(tile.TileFrameX, tile.TileFrameY, 16, tile.IsHalfBlock ? 8 : 16), Color.White * 0.6f);
                                    }
                                }
                            }
                        }
                        ...
                        {
                            int num = 4;
                            if (tilesToDraw.TryGetTile(new Point16(tileCoord.X, tileCoord.Y - 1), out TileCopy above) && (tile.LiquidAmount < 255 || !above.HasTile) && above.LiquidAmount == 0) // determite if the surface part of the liquid should be shown
                            {
                                num = 0;
                            }
                            ...
                            SpriteBatch.Draw(TextureAssets.Liquid[tile.LiquidType].Value, boundsTile, new Rectangle(0, num, 16, height), Color.White * 0.6f);
                        }
                        ...
                        if (drawWires && (tile.GreenWire || tile.RedWire || tile.YellowWire || tile.BlueWire))
                        {
                            ...
                            if (tilesToDraw.TryGetTile(new Point16(tileCoord.X, tileCoord.Y - 1), out TileCopy top) && HasMatchingWire(tile, top))
                            {
                                topWire = top.HasWire;
                            }
                            if (tilesToDraw.TryGetTile(new Point16(tileCoord.X, tileCoord.Y + 1), out TileCopy bottom) && HasMatchingWire(tile, bottom))
                            {
                                bottomWire = bottom.HasWire;
                            }
                            if (tilesToDraw.TryGetTile(new Point16(tileCoord.X + 1, tileCoord.Y), out TileCopy right) && HasMatchingWire(tile, right))
                            {
                                rightWire = right.HasWire;
                            }
                            if (tilesToDraw.TryGetTile(new Point16(tileCoord.X - 1, tileCoord.Y), out TileCopy left) && HasMatchingWire(tile, left))
                            {
